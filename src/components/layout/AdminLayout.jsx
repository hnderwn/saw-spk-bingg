...
const AdminLayout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex" style={{ backgroundColor: '#F2ECD8', fontFamily: "'DM Sans',sans-serif" }}>
      ...
      {/* ══════════ SIDEBAR ══════════ */}
      <aside
        className="fixed h-full z-20 flex flex-col transition-all duration-300"
        style={{
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-3 space-y-0.5"
          style={{ padding: isSidebarOpen ? '12px 10px' : '12px 8px' }}>
          {menuItems.map((item) => {
              ...
              <Link
                ...
                style={{
                  ...
                  borderLeft: isActive
                    ? '3px solid #C9A84C'
                    : isSiswa
                    ? '3px solid rgba(191,10,48,0.3)'
                    : '3px solid transparent',
                  color: isActive
                    ? '#C9A84C'
                    : isSiswa
                    ? 'rgba(252,205,211,0.8)'
                ...
                onMouseLeave={e => {
                  if (!isActive) {
                    ...
                    e.currentTarget.style.color = isSiswa
                      ? 'rgba(252,205,211,0.8)'
                      : 'rgba(255,255,255,0.6)';
                  }
                }}
        ...
        <div style={{ borderTop: '1px solid rgba(201,168,76,0.15)', padding: isSidebarOpen ? '12px 10px' : '12px 8px', flexShrink: 0 }}>
          {/* Profile card */}
          {isSidebarOpen && (
            <div className="mb-2 px-3 py-2.5 rounded-sm"
        ...
        {/* Toggle button — logika asli tidak diubah */}
        <button
          onClick={() => setIsSidebarOpen(!isSidebarOpen)}
          className="absolute -right-3 top-24 w-6 h-6 rounded-full flex items-center justify-center transition-all hover:scale-110 active:scale-95 z-30"
          style={{
            background: '#FAF6EC',
